...
customer * read_customer_txt() //读取customer。txt内容 
{
	...
	int i=0;
	char b;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\Windows\\现代操作系统实验\\第一次\\small\\customer.txt","r");
	if(NULL==fp)
	{
...
orders * read_orders_txt()//读取orders.txt内容 
{
	...
	long long d;
	FILE *fp;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\Windows\\现代操作系统实验\\第一次\\small\\orders.txt","r");
	if(fp == NULL)
	{
...
lineitem * read_lineitem_txt()//读取lineitem.txt内容
{
	...
	int i=0;
	char b,c;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\Windows\\现代操作系统实验\\第一次\\small\\lineitem.txt","r");
	if(fp==NULL)
	{
